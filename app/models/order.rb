class Order < ActiveRecord::Base
	 ...
	 def description
	 	if(self.tipo=="demolay")
	 		...
	 		"#{self.tipo.capitalize}, #{self.operation.capitalize}: #{number_to_currency self.price} x #{self.qtd_membro} = #{number_to_currency result} | Responsável: #{self.name}, CPF: #{self.cpf}, CID: #{self.cid} | Protocolo: #{self.protocolo}"
	 	end
	 end
end